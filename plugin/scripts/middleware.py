...
class Sender:
    ...
    def input(self,timeout,recallDuration,forGroup):
        path="/input"
        params={
            "senderid":self.senderID,
            "timeout":timeout,
...
class Cron:
    ...
    # 添加定时，返回定时指令的ID
    def addCron(self,cron,cmd,isToSelf,toOthers,memo):
        path="/croncmdsAdd"
        params={
            "cron":cron,
            "cmd":cmd,
        ...
        return response.text
    # 修改定时，返回ok或error
    def updateCron(self,id,cron,cmd,isToSelf,toOthers,memo):
        path="/croncmdsUpd"
        params={
            "id":id,
            "cron":cron,
