$(document).ready(function () {
    ...
    lightyear.notify('AUT云市场支持1小时首装试用，插件安装1小时内卸载会自动退款', 'warning', 10000, 'mdi mdi-emoticon-happy', 'top', 'center');
})
...
function bought() {
    //alert("调用我的")
    $.get("/js/bought", function (response) {
        //console.log(response)
        $.alert({
            title: '已购买',
        ...
        if (response.code == 200) {
            ...
            switch (rank) {
                ...
                case "download"://下载排行
                    ...
                    for (var i = 0; i < obj.length - 1; i++) {
                        for (var j = 0; j < obj.length - 1 - i; j++) {
                            if (obj[j].download < obj[j + 1].download) {
                                tmp = obj[j]
                                obj[j] = obj[j + 1]
                                obj[j + 1] = tmp
                            }
...
function fillPlugins(cloudObjs, localObjs) {
    ...
    for (var i = 0; i < obj.length; i++) {
        ...
        if (isCloud) {
            //按钮的class
            btnClass = `form-control btn-round btn-primary`
            //图标拉起不同的显示框
            divMethod = `data-toggle="modal" data-target="#myModalDescript" onclick="fillModalDescript(\`` + encodeURIComponent(JSON.stringify(obj[i])) + `\`)"`
            //评论拉起不同的显示框
            divComment = `data-toggle="modal" data-target="#myModalAppraise" onclick="fillModalAppraise('${obj[i].title}')"`
            //下载数据
            download = `<p class="card-text m-t-5 m-b-5"><b>下载：</b>` + obj[i].download + `</p>`
            //更新时间数据
            updatedTime = `<p class="card-text m-t-5 m-b-5"><b>更新：</b>${obj[i].created_at}</p>`
            //按钮文字
                ...
                for (j = 0; j < localObj.length; j++) {
                    if (obj[i].title == localObj[j].title) {
                        if (obj[i].version != localObj[j].version) {
                            btnValue = "更新"
                            btnClass = `form-control btn-round btn-success`
                        } else {
                            btnValue = "卸载"
                            btnClass = `form-control btn-round btn-danger`
                        }
                        break
                    }
                }
            ...
            if (rating == 0.0) {
                score = '<i class="mdi mdi-approval"></i> 暂无'
            } else {
                score = '<i class="mdi mdi-approval"></i> ' + parseFloat(obj[i].rating).toFixed(1)
            }
            ...
            //按钮
            btnClass = `form-control btn-round btn-danger`
            divMethod = `data-toggle="modal" data-target="#myModalSettings" onclick="fillModalSettings(\`` + encodeURIComponent(JSON.stringify(obj[i])) + `\`)"`
            divComment = `data-toggle="modal" data-target="#myModalRating" onclick="fillModalRating('${obj[i].title}')"`
            ...
            class="form-control btn-round btn-primary m-t-10" 
            value="配参" 
            data-toggle="modal" 
            data-target="#myModalParams" 
                        ...
                        <div class="card ${divCardClass}">
                            <div class="row small">
                                ...
                                <div class="col-sm-3 col-lg-3" ${divComment}>
                                    ...
                                    <p class="card-text m-t-5 m-b-0">${score}</p>
                                    <p class="card-text m-t-0 m-b-0">${openSource}</p>
                                </div>

                                <!-- 标题和介绍 -->
                                <div class="col-sm-6 col-lg-6" ${divMethod}>
                                    <p class="card-text m-t-5 m-b-5"><b>名称：</b>${obj[i].title}</p>
                                    <p class="card-text m-t-5 m-b-5"><b>介绍：</b>${obj[i].description}</p>
                                    ${download}
                                    ${updatedTime}
                                </div>
...
function submitModalRating() {
    ...
    if ($("#modalRating [name='comment']").val() == "") {
        lightyear.notify("请填写评价内容", 'danger', 3000);
        return
    }
    // //判断星级是否选择
    // if ($("#[name='rating']:checked").length > 0) {
    //     lightyear.notify("请选择星级", 'danger', 3000);
    //     return
    // }
    ...
    $.ajax({
        ...
        success: function (response) {
            console.log(response)
            if (response.code == 200) {
                lightyear.notify("评价成功", 'success', 1000);
                $("#myModalRating").modal('hide');
            } else {
                lightyear.notify("评价失败：" + data, 'danger', 3000);
            }
        }
    })
}
            ...
            html += `<div class="form-group">
                        ...
                        <div class="col-sm-9">
                            <label class="lyear-switch switch-solid switch-primary">
                                <input type="checkbox" id="${objs[i].key}" name="${objs[i].key}">
                                <span></span>
                            </label>
                            <small class="help-block">${objs[i].desc}</small>
                        </div>
                    </div>`
        } else {//文本框
            html += `<div class="form-group">
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            if (response.code == 200) {
                ...
                lightyear.notify("保存失败：" + data, 'danger', 3000);
            }
        }
    })
}
...
function fillModalAppraise(title) {
    ...
    $.ajax({
        ...
        error: function (XMLHttpRequest, textStatus, errorThrown) {
            ...
            lightyear.notify('获取评价失败', 'danger', 3000);
        }
    });
}
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.code == 200) {
                    $("#" + name).val("安装成功")
                    $("#" + name).val("卸载")
                    $('#' + name).attr('class', 'form-control btn-round btn-danger');
                } else {
                    $("#" + name).val("安装失败")
                    $("#" + name).val(oldValue)
                    $('#' + name).attr('class', 'form-control btn-round btn-primary');
                    //alert(result)
            ...
            content: '确定删除？' +
                ...
                <div class="form-group">
                    <label class="lyear-checkbox checkbox-inline checkbox-primary">
                        <input id="delconfig" type="checkbox"><span>同时删除配置</span>
                    </label>
                    <label class="lyear-checkbox checkbox-inline checkbox-primary">
                        <input id="delparams" type="checkbox"><span>同时删除配参</span>
                    </label> 
                </div>
                </form>`,
            buttons: {
